...
export class RequestToJoinService {
  private baseUrl = 'http://localhost:8080/api/requestToJoin';  // Update this URL
  ...
  getRequestsToJoinByUserId(userId: number): Observable<RequestToJoin[]> {
    return this.http.get<RequestToJoin[]>(`${this.baseUrl}/getByUserId/${userId}`);
  }
  
  getRequestsToJoinByClubId(clubId: number): Observable<RequestToJoin[]> {
    return this.http.get<RequestToJoin[]>(`${this.baseUrl}/getByClubId/${clubId}`);
  }

  updateRequestToJoin(requestToJoin: RequestToJoin): Observable<RequestToJoin> {
    return this.http.put<RequestToJoin>(`${this.baseUrl}/update`, requestToJoin);
  }
}